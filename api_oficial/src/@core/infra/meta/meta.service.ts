...
export class MetaService {
    ...
    urlMeta = `https://graph.facebook.com/v19.0`;
    ...
    async authFileMeta(idMessage: string, phone_number_id: string, token: string): Promise<IReturnAuthMeta> {
        try {
            ...
            const url = `https://graph.facebook.com/v19.0/${idMessage}?phone_number_id=${phone_number_id}`;
    ...
    async downloadFileMeta(idMessage: string, phone_number_id: string, token: string, companyId: number, conexao: number): Promise<{ base64: string, mimeType: string }> {
        ...
        try {
            ...
            const headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, "User-Agent": "curl/7.64.1" };
