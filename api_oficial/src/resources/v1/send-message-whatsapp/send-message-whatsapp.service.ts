...
import { IBodyReadMessage, IMetaMessage, IMetaMessageAudio, IMetaMessageContacts, IMetaMessageDocument, IMetaMessageImage, IMetaMessageLocation, IMetaMessageReaction, IMetaMessageSticker, IMetaMessageTemplate, IMetaMessageText, IMetaMessageVideo, IMetaMessageinteractive } from 'src/@core/infra/meta/interfaces/IMeta.interfaces';
...
export class SendMessageWhatsappService extends BaseService<SendMessageWhatsApp> {
  ...
  private async createFile(file: Express.Multer.File, fileName: string, empresaId: number, conexaoId: number) {
    try {
  ...
  private async getIdMetaMedia(whatsId: number, phone_number_id: string, token: string, idCompany: number, file: Express.Multer.File, fileName: string) {
    try {
  ...
  async createMessage(token: string, dados_mensagem: string, file: Express.Multer.File) {
    try {

      const data: CreateSendMessageWhatsappDto = JSON.parse(dados_mensagem);
      const regex = /^\+55\d{2}\d{8,9}$/;

      if (!data.to) throw new Error('Necessário informar o número do destinatario');
      if (!regex.test(data.to)) throw new Error('o número não está no padrão do whatsapp');

      const whats = await this.prisma.whatsappOficial.findFirst({ where: { token_mult100: token } });

      if (!whats) throw new Error('Conexão não encontrada');

      const company = await this.prisma.company.findFirst({ where: { id: whats.companyId } });

      if (!company) throw new Error('Nenhuma empresa cadastrada para este usuário')

      const entity: SendMessageWhatsApp = { type: data.type, whatsappOficialId: whats.id, to: data.to };
      ...
      switch (data.type) {
        case 'text':
          if (!body_text.body) throw new Error('Necessário informar um texto para enviar a mensagem');

          entity.text = { body: body_text.body, preview_url: body_text?.preview_url };
        ...
        case 'image':
          ...
          break;
        case 'location':
          if (!body_location.latitude && !body_location.longitude) throw new Error('Necessário informar a latitude e longitude');
          ...
          break;
        case 'reaction':
          if (!body_reaction.message_id || !body_reaction.emoji) throw new Error('Necessário informar o id da mensagem e o emoji');

          entity.reaction = { message_id: body_reaction.message_id, emoji: body_reaction.emoji };

          dataMessage = { message_id: body_reaction.message_id, emoji: body_reaction.emoji };
          break;
        case 'contacts':
          ...
          break;
        case 'interactive':
          if (body_interactive.type == 'button' || body_interactive.type == 'list') {
          ...
          break;
        case 'sticker':
          if (!body_sticket.id) throw new Error('Necessário informar o id do sticker');
  ...
  async readMessage(token: string, messageId: string) {
    try {
      ...
      const whats = await this.whatsAppService.prisma.whatsappOficial.findUnique({ where: { token_mult100: token } });
