...
import { IWebhookWhatsApp, IWebhookWhatsAppEntryChangesValueMessages } from './interfaces/IWebhookWhatsApp.inteface';
import { SocketService } from 'src/@core/infra/socket/socket.service';
import { IMessageReceived, IReceivedWhatsppOficial } from 'src/@core/interfaces/IWebsocket.interface';
...
export class WebhookService {
  ...
  private messagesPermitidas = ['text', 'image', 'audio', 'document', 'video', 'location', 'contacts', 'order', 'interactive', 'referral'];
  private tipoLiberadosParaMult100 = ['text', 'image', 'audio', 'document', 'video', 'location', 'contacts']
  ...
  async sendToWebhook(webhook_url: string, token: string, body: any) {
    try {
      ...
      this.logger.log("Resposta do encaminhamento do webhook", { webhook_url, responseData });
    } catch (error: any) {
      this.logger.error("Erro ao encaminhar para o webhook", { erro: error.message, webhook_url });
      return null;
    }
  }
  ...
  async webhookCompanyConexao(companyId: number, conexaoId: number, data: any) {
    try {
      ...
      const company = await this.whatsAppService.prisma.company.findUnique({ where: { id: companyId } });
      ...
      if (body.object == 'whatsapp_business_account') {
              ...
              if (value?.statuses != null) {
                ...
                for (const message of value.messages) {
                  if (this.messagesPermitidas.some(m => m == message.type)) {
                    ...
                    if (!!messages) {
                      const messagesStored: Array<any> = (JSON.parse(messages) as Array<any>);
                    ...
                    switch (message.type) {
                      ...
                      default:
                        file = null;
                        bodyMessage = message.type === "contacts" ? {
                          contacts: message.contacts
                        } : message.text.body;
                        break;
                    }

                    if (this.tipoLiberadosParaMult100.some(m => m == message.type)) {
        ...
        this.logger.error(`Evento não tratado: ${body}`);
      }
  ...
  async webhookCompany(companyId: number, conexaoId: number, mode: string, verify_token: string, challenge: string) {
    try {
