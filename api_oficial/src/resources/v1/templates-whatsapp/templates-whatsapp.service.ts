...
export class TemplatesWhatsappService {
  ...
  async findAll(token: string) {
    try {
      const conexao = await this.whatsappOficial.prisma.whatsappOficial.findUnique({ where: { token_mult100: token, deleted_at: null } });

      if (!conexao) throw new Error('Nenhuma conexão existente com este id');

      return await this.metaService.getListTemplates(conexao.waba_id, conexao.send_token);
