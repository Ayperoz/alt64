...
import { CreateCompanyConnectionOficial, DeleteConnectionWhatsAppOficial, getTemplatesWhatsAppOficial, loginWhatsAppOficial, UpdateConnectionWhatsAppOficial } from "../libs/whatsAppOficial/whatsAppOficial.service";
  ...
  if (["whatsapp_oficial"].includes(whatsapp.channel)) {
    try {
      ...
      const whatsappOficial: ICreateConnectionWhatsAppOficialWhatsApp = {
        ...
        bussines_id: whatsapp.business_id,
      ...
      const webhook = await CreateCompanyConnectionOficial(data)

      if (webhook) {
        whatsapp.waba_webhook = webhook
...
export const update = async (
  ...
  if (["whatsapp_oficial"].includes(whatsapp.channel)) {
    try {
      ...
      await UpdateConnectionWhatsAppOficial(whatsapp.company.email, whatsappOficial)
...
export const remove = async (
  ...
  if (whatsapp.channel === "whatsapp_oficial") {
    ...
    try {
      ...
      await DeleteConnectionWhatsAppOficial(whatsapp.company.email)
      ...
      if (quickMessage) {
        ...
        if (template?.components?.length > 0) {
          if (quickMessage?.components?.length > 0) {
            ...
            console.log('QuickMessage has no components');
          }
