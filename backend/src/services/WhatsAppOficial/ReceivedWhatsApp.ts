...
export function generateVCard(contact: any): string {
    const firstName = contact.name.split(' ')[0];
    const lastName = String(contact.name).replace(contact.name.split(' ')[0], '')
...
export class ReceibedWhatsAppService {
    ...
    async getMessage(data: IReceivedWhatsppOficial) {
        try {
            ...
            const { file, mimeType, idFile, type } = message;
            ...
            const ticket = await FindOrCreateTicketService(contact, whatsapp, 0, companyId, null, null, null, 'whatsapp_oficial', false, false, settings, false, false);
            ...
            await verifyMessageOficial(message, ticket, contact, companyId, fileName, fromNumber, data);
    ...
    async readMessage(data: IReceivedReadWhatsppOficialRead) {
        ...
        try {
            ...
            const message = await Message.findOne({ where: { wid: messageId, companyId: conexao.companyId } });
